...
\begin{tikzpicture} [
    ...
    \node (x1) [plain] at (\xarray[0], \yarray[0]) {$\xv_1$};
    \node (x2) [plain] at (\xarray[1], \yarray[0]) {$\xv_2$};
    \node (x3) [plain] at (\xarray[2], \yarray[0]) {...};
    \node (x4) [plain] at (\xarray[3], \yarray[0]) {$\xv_T$};
    ...
    \foreach \i in {1,2,4}
        {
            ...
            \draw[->] (a\i) -- (t\i);
            \draw[->] (t\i) -- (add);
            \draw[->,Solarized-yellow,dashed] (x\i) -- ++(1.2, 0) -- ++(0, 5.4) -- (t\i);
        }
