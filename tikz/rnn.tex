...
\begin{tikzpicture} [thick]
    ...
    \node[rectangle,draw=Solarized-base01,fill=Solarized-base2,minimum size=20] (o) at (0,4) {输出层};

    \node[rectangle,draw=Solarized-base01,minimum size=20] (a) at (0,2) {隐藏层};

    \node[rectangle,draw=Solarized-base01,minimum size=20] (d) at (4,2) {延迟器};

    \node[rectangle,draw=Solarized-base01,fill=Solarized-base2,minimum size=20] (i) at (0,0) {输入层};

    \draw[->] (i) -- (a) node [midway, draw=none, left] {$\xv_t$};

    \draw[->] (a) -- (o) node [midway, draw=none, left] {$\av_t$};

    \draw[->] (a) to [out=45,in=135] (d);
    \node[above] at (2,3.1) {$\av_t$};

    \draw[->] (d) to [out=225,in=315] (a);
