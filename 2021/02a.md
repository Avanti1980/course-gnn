...
tf = 单词在当前文本中出现的次数 / 当前文本的总词汇数

idf = ln ((全部文本数 + C) / (包含该词的文本总数 + C)) + 1

- C = 0，原始版本
- C = 1，sklearn 中默认采用的平滑版本，相当于额外有一个包含所有词的文本

tf - idf 特征 = normalize (tf × idf)

- $\ell_1$规范化，tf × idf / sum (tf × idf)，变成一个概率分布
- $\ell_2$规范化，tf × idf / sqrt(sum ([tf × idf]^2))
...
序数编码 (ordinal encoding)：浊响 - 0、清脆 - 1、沉闷 - 2

- 人为引入了序，例如清脆是浊响和沉闷的中间值，这很没道理

独热编码 (one-hot encoding)：浊响 - 001、清脆 - 010、沉闷 - 100

- 对所有取值一碗水端平，若取值很多码会很长，且不适应动态出现的新取值

散列编码 (hash encoding)：用哈希函数将任意输入映射到有限整数范围

- 码长固定，也能适应动态出现的新取值，但可能存在信息丢失
