...
- 取$\sigma(\wv^\top \xv + b)$为预测为正类的概率，则$1 - \sigma(\wv^\top \xv + b)$为预测为负类的概率
- 预测结果分布可记为$\pv = [\sigma(\wv^\top \xv + b); 1 - \sigma(\wv^\top \xv + b)]$
- 由于$y \in \{ \pm 1 \}$，真实类别分布可记为$\qv = [\frac{1+y}{2}; \frac{1-y}{2}]$
- 取$\pv$和$\qv$的<span class="blue">交叉熵</span>作为损失 ($\pv = \qv$时取最小)
