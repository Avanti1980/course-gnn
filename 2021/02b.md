...
问题：如何让模型的学习只出现内积$\phi(\xv_i)^\top \phi(\xv_j)$的形式？
...
记$\Kv = \phi(\Xv) \phi(\Xv)^\top = \begin{bmatrix} \phi(\xv_1)^\top \phi(\xv_1) & \cdots & \phi(\xv_1)^\top \phi(\xv_m) \\ \vdots & \ddots & \vdots \\ \phi(\xv_m)^\top \phi(\xv_1) & \cdots & \phi(\xv_m)^\top \phi(\xv_m) \end{bmatrix}$，于是
...
核 PCA 可重写为$\max_\alphav \alphav^\top \Kv \Kv \alphav, \quad \st ~ \alphav^\top \Kv \alphav + \vv^\top \vv = 1$
...
$$
\begin{align*}
    \text{核 PCA}: \max_{\|\wv\|_2^2 = 1} \left\| \begin{bmatrix} \phi(\xv_1)^\top \wv \\ \vdots \\ \langle \phi(\xv_m)^\top \wv \end{bmatrix} \right\|_2^2 = \| \Kv \alphav \|_2^2 = \alphav^\top \Kv \Kv \alphav
\end{align*}
$$

可重写为$\max_\alphav \alphav^\top \Kv \Kv \alphav, \quad \st ~ \alphav^\top \Kv \alphav + \langle \vv, \vv \rangle = 1$

$$
\begin{align*}
    \text{核 PCA}: ~ \max_\alphav ~ \| \Kv \alphav \|_2^2 = \alphav^\top \Kv \Kv \alphav \quad \st ~ \alphav^\top \Kv \alphav + \langle \vv, \vv \rangle = 1
\end{align*}
$$
