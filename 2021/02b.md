...
<div class="threelines column-1-border-right">
...
<img src="../python/binary-classif.svg" title="第一列是三个人工数据集，第一个呈半月形，第二个呈圆环型，第三个线性可分，两种颜色深色的为训练样本，稍浅些的为测试样本，每个数据集各有20%的噪声；右边三列为线性核支持向量机、高斯核支持向量机、神经网络在这些数据集上的分类结果，右下角是预测精度；前两个数据集非线性可分，后两个分类器非线性分类器，注意对比观察它们的效果" class="center top2 bottom0 width24">
...
<img src="../python/multi-classif-train-prediction.svg" title="手写数字识别数据" class="center top2 width30">
...
<img src="../python/multi-classif-confusion-matrix.svg" title="混淆矩阵" class="center top2 width20">
...
<img src="../python/regression.svg" title="" class="center top3 bottom-10 width20">
...
<div class="threelines column-1-border-right row-4-border-top-dashed">
...
<img src="../python/clustering.svg" title="" class="center top3 bottom-10 width20">
...
<img src="../python/density-estimation-1.svg" title="" class="center top3 bottom-10 width20">
...
<img src="../python/density-estimation-3.svg" title="" class="center top3 bottom-10 width20">
...
<img src="../tikz/overfitting.svg" title="随着假设空间复杂度地不断增大，对训练数据拟合地越来越好，经验风险单调下降，但泛化风险却是先降后升" class="width18 left12 top2 bottom2">
...
<div class="threelines column-1-border-right  column-1-bold">
...
方案：用另一个函数$l(y, f(\xv))$替代，称为<a target="_blank" href="https://en.wikipedia.org/wiki/Loss_functions_for_classification">替代损失</a>，常见的有
...
<img src="../python/gradient-descent.svg" title="用梯度下降求解R上的最小二乘" class="center top6 width30">
